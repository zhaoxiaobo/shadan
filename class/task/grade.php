...
class task_grade{
	...
	public static function review($tid, $uid, $review,$type = 0){
		if ($task = db::one('task', 'suid,buid', "id='$tid'")) {
			if ($gradeInfo = self::get($tid, $uid)) {
				//已经评价过的，修改
				if (db::update('credits', array(
					...
					'review'    => $review
						...
						$msg = '您在'.language::get('qu'.$task['type']).'区接的任务“'.$task['id'].'”，增加了追评！';
						member_base::sendPm($task['buid'], $msg, '任务“'.$task['id'].'”，增加了追评', 'in_grade');
						member_base::sendSms($task['buid'], $msg, 'in_grade');
				}
			} 
		}
