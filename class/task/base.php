...
class task_base{
	...
	public static function tieBuyer($tid, $bid, $uid) {
		global $timestamp, $today_start, $today_end, $tswkStart, $tswkEnd, $tskStart, $tsmEnd,$ipint, $isVip;
		if ($tid && $bid && $uid) {
					...
					//if ($task['type'] == $buyer['type'] || (in_array($task['type'], array(4, 5, 6)) && $buyer['type'] == 1)) {
							if (in_array($buyer['status'], array(0, 1))) {
								...
								if (!in_array($task['type'], array(6))) {//非会员功能
								    // 每个买号每日最多接手6个任务
									$inCount = db::data_count('task', "bid='$bid' and (btimestamp>=$today_start and btimestamp<=$today_end)");
									$dayCount=cfg::getInt('sys', 'sys_dayCount')-1;
									if ($inCount >$dayCount) return '对不起，同一买号，一天内只能接手'.cfg::getInt('sys', 'sys_dayCount').'个任务';
									// 每个买号每周最多接手任务个数
									$weekCount=cfg::getInt('sys', 'sys_weekCount')-1;
									$t1 = $today_start - 6 * 86400;
									$t2 = $today_end;
									$inCount = db::data_count('task', "bid='$bid' and (btimestamp>=$t1 and btimestamp<=$t2)");
									if ($inCount >$weekCount) return '对不起，同一买号，一周内只能接手'.cfg::getInt('sys', 'sys_weekCount').'个任务';
									// 1个IP一天内 只允许 一个接手人账号 只能接手 一个掌柜的 一件商品
									$inCount = db::data_count('task', "sid='$task[sid]' and bid='$bid' and (btimestamp>=$today_start and btimestamp<=$today_end) and bip='$ipint'");
						            if ($inCount >0) return '对不起，同一个IP在24小时内不允许使用不同的买号绑定到同一个店铺的任务';
								 	//14天之内只能接手同一卖家一次任务
									...
									if ($inCount >0) return '对不起，同一个买号一个任务宝贝在30天只能接手一次';

									//同一买号不能在三十天内接同一发布人同一网店任务超过三个
									$t1 = $today_start - 29 * 86400;
									$t2 = $today_end;
									$inCount = db::data_count('task', "sid='$task[sid]' and bid='$bid' and (btimestamp>=$t1 and btimestamp<=$t2)");
									if ($inCount > 2) return '对不起，同一个买号30天内只能接手同一个卖家店铺不大于3个宝贝';
								}
								if ($task['isStar']) {
								...
								if ($buyer['score'] > $minScore) {
									...
									$jierenwutimex = cfg::getInt('sys', 'jierenwutime');
									db::update('task', array(
										'bid'        => $bid,
										'bnickname'  => $buyer['nickname'],
										'ttimestamp' => $task['isVerify']?0:$timestamp + $jierenwutimex * 60,//后台设置时间
