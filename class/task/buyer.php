...
class task_buyer{
	...
	public static function tie($uid, $type, $nickname,$isrealsj){
		global $timestamp;
	...
	public static function getList($type, $uid, $status = 0, $spage = 0, $spagesyze = 0) {
		...
		if ($list = db::get_listbuy('buyers', '*', $where, 'status,timestamp desc', $spage, $spagesize)) {
			$icoFunc = 'getIco'.$type;
			foreach ($list as $k => $v) {
...
*/
	...
	public static function getIco3($score){
		global $weburl2;
		$img = '';
		$level = 0;
		if ($score > 0) {
...
*/
	...
	public static function getIco4($score){
		global $weburl2;
		$img = '';
		$level = 0;
		if ($score > 0) {
