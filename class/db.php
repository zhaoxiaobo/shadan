...
class db{
	...
	public static function get_listbuy($tbname,$f='*',$where='',$order='',$page=1,$pagesize=200){
		global $db,$pre;
		$where && $where=' where '.$where;
		$order && $order=' order by '.$order;
		$limit = '';
		$page > 0 && $limit = ' limit '.($page-1) * $pagesize.','.$pagesize;
		if($ids=$db->fetch_first_all("select id from $pre$tbname$where$order")){
			return $db->fetch_all("select $f from $pre$tbname where id in('".implode('\',\'',$ids)."')$order");
		}
	}
	...
	public static function get_list4($tbname,$f='*',$where='',$order='',$page=1,$pagesize=3000){
		global $db,$pre;
		$where && $where=' where '.$where;
		$order && $order=' order by '.$order;
		$limit = '';
