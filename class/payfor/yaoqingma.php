...
class payfor_yaoqingma{
	...
	public function createCard($count, &$cardList = false){
		global $db, $pre, $timestamp;
		$c = 0;
		$return = $cardList !== false;
		if ($return) $cardList = array();
		$db->query("lock table `{$pre}yaoqingma` write");
		for ($i = 0; $i < $count; $i++) {
			do {
				$id = $this->createId();
			} while(db::exists('yaoqingma', array('id' => $id)));
			$key = $this->createKey();
			$cardList[] = array('id' => $id, 'key' => $key);
			if (db::insert('yaoqingma', array(
				'id'         => $id,				
				'pwd'        => $key,
	...
	public function useCard($id, $pwd){
		global $timestamp;
		if ($card = db::one('yaoqingma', '*', "id='$id'")) {
			if ($card['pwd'] == $pwd) {
				if (!$card['status']) {
					db::update('yaoqingma', array('status' => 1, 'utimestamp' => $timestamp), "id='$id'");
					return $card['money'];
				}
				return 'yaoqingma_is_used';
			}
			return 'yaoqingma_pwd_error';
		}
		return 'yaoqingma_not_exists';
	}
}
?>