...
class payfor_topup{
	...
	public static function payfor3($uid, $money1, $type, $useMp = true, $datas = array()){
		global $db, $pre, $timestamp;
		self::$status = false;
		if (member_base::memberIdExists($uid)) {
			$class = 'payfor_'.$type;
	...
	public static function complateOrderalipay($id,$pay_money, $remark = ''){
		global $timestamp, $web_name;
		if ($order = db::one('topup', '*', "id='$id'")) {
			if (!$order['status']) {
				//还没有充值成功的
				if ($money = member_base::addMoney($order['uid'], $pay_money, $remark)) {
					$credit = 0;
					$vip = member_base::isVip($order['uid']);
					if ($pay_money >= 5000) {
						if ($vip) $credit = 600;
					...
					} elseif ($pay_money >= 500) {
						if ($vip) $credit = 50;
						else $credit = 40;
					}
					if ($credit > 0) {
						$credit = member_base::addCredit($order['uid'], $credit, '充值奖励','充值平台');
					}
					db::update('topup', array(
						'money'      => $pay_money,
						'credit'     => $credit,
					...
					member_base::sendPm($order['uid'], '恭喜您，您成功充值了'.$pay_money.'元存款', '网站提醒：充值成功', 'remit');
					member_base::sendSms($order['uid'], '恭喜您，您成功充值了'.$pay_money.'元存款', 'remit');
				}
				return true;
			}
		}
...
<body>'.$html.'</body>
	...
	private static function formatData3($args){
		if (!is_array($args) || !$args['url'] || !$args['type']) return false;
		$args['encoding'] || $args['encoding'] = ENCODING;
		$html = '';
		if ($args['type'] == 'post') {
			...
			$fukuanshuomingx = 'bu yao xiu gai fu kuan shuo ming';
			
			$html = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset='.$args['encoding'].'" /><form method="post" action="'.$args['url'].'" enctype="application/x-www-form-urlencoded" name="myForm"><input name="optEmail" id="optEmail"  value="'.$args['username'].'"  type="hidden"><input name="payAmount" id="payAmount" value="'.$args['alipayMoney'].'" type="hidden"><input name="title" id="title" value="'.$args['v_oid'].'" type="hidden" /><textarea id="comment" name="memo" class="i-textarea" seed="comments-area">'.$fukuanshuomingx.'</textarea></form>
			<script language="javascript">document.myForm.submit();</script>
			';
		} elseif ($args['type'] =='get') {
			$url = '';
