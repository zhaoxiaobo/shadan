	...
	public static function delModerator2($fid, $uid){
		if (self::forumExists($fid)) {
			...
			if ($mid = db::one_one('moderator', 'id', "fid='$fid' and uid='$uid'")) {
				...
				if($m['credits']==0){
		             $credits='novice_users';
		        }elseif($m['credits'] < 100){
		             $credits='gold_users';
		        }elseif($m['credits'] < 1000){
		            $credits='ordinary_users';
		        }elseif($m['credits'] < 5000){
		             $credits='platinum_users';
		        }elseif($m['credits'] < 10000){
		              $credits='diamond_users';
		        }elseif($m['credits'] < 100000){
		              $credits='皇冠会员';
		        }
				if($rank){
				     $novice = $rank;
