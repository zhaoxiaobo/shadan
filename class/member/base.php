...
class member_base{
	...
	public static function verify($datas, $uid_ = 0){

		global $uid, $ipint, $timestamp;
		...
		$idcard = trim($datas['idcard']);

		$username = trim($datas['truename']);

		$usernameh = trim($datas['usernameh']);
		...
		preg_match_all('/\[attach\](\d+)\[\/attach\]/', $remark, $pinimagexinxi);

		$pinimageid = $pinimagexinxi[1];

		if($pinimageid){

			foreach ($pinimageid as $value){									

				$pinimagetu = db::one('attach', 'src', "id='$value'");

				$pinimageurl = '[img]/img/attach/'.$pinimagetu['src'].'[/img]';	

				$valueyao = '[attach]'.$value.'[/attach]';							

				$remark = str_replace($valueyao,$pinimageurl,$remark,$i);
		...
		preg_match_all('/\[attach\](\d+)\[\/attach\]/', $remark1, $pinimagexinxi1);

		$pinimageid1 = $pinimagexinxi1[1];

		if($pinimageid1){

			foreach ($pinimageid1 as $value1){									

				$pinimagetu1 = db::one('attach', 'src', "id='$value1'");

				$pinimageurl1 = '[img]/img/attach/'.$pinimagetu1['src'].'[/img]';	

				$valueyao1 = '[attach]'.$value1.'[/attach]';							

				$remark1 = str_replace($valueyao1,$pinimageurl1,$remark1,$i);
		...
		preg_match_all('/\[attach\](\d+)\[\/attach\]/', $remark2, $pinimagexinxi2);

		$pinimageid2 = $pinimagexinxi2[1];

		if($pinimageid2){

			foreach ($pinimageid2 as $value2){									

				$pinimagetu2 = db::one('attach', 'src', "id='$value2'");

				$pinimageurl2 = '[img]/img/attach/'.$pinimagetu2['src'].'[/img]';	

				$valueyao2 = '[attach]'.$value2.'[/attach]';							

				$remark2 = str_replace($valueyao2,$pinimageurl2,$remark2,$i);
		...
		$avatarzong = $remark.$remark1.$remark2;



		if(db::update('memberfields', 'isVerify = 2', "uid='$uid_'")){

			db::insert('verify', array(
	...
	public static function checkMemberData(&$datas){
		...
		if (is_array($datas)) {
			...
			if ($datas['username'] && $datas['password'] && $datas['password2'] && $datas['truename'] && $datas['qq'] && $datas['email'] && $datas['mobilephone'] && $datas['sex']) {
				...
				if (db::exists(self::$tbName, array('username'   => $datas['username'])))  return 'username_exists';
	...
	public static function addFabudian3($uid, $nums, $type, $remark = ''){
		...
		if (self::memberIdExists($uid)) {
			...
			db::update('memberfields', 'fabudian3=fabudian3'.($nums >= 0?'+':'').$nums, "uid='$uid'");

			self::addLog('fabudian3', $uid, $nums, $remark);

			return db::one_one('memberfields', 'fabudian3', "uid='$uid'");
	...
	public static function addPoint($tid, $point = 0){
		...
		if ($task = db::one('task', '*', "id='$tid'")) {
			...
			if ($member = db::one('memberfields', '*', "uid='$uid'")) {
				...
				$koudianjifenzhi = cfg::getInt('sys', 'koudianjifen');

				if ($member['credits'] >= $koudianjifenzhi) {
				...
				self::addFabudian($uid, $point, $task['type'], '完成任务'.$task['id'].'奖励信币');
	...
	public static function backPoint($tid, $point = 0){
		...
		if ($task = db::one('task', '*', "id='$tid'")) {
			...
			if ($member = db::one('memberfields', '*', "uid='$uid'")) {
				...
				self::addFabudian($uid, $point, $task['type'], '返还任务'.$task['id'].'的信币');
	...
	public static function getMemberFields($uid){
		...
		if ($fields = db::one('memberfields', '*', "uid='$uid'")) {
			...
			$fields['allgrade'] = $fields['sgrade'] + $fields['bgrade'];
	...
	public static function deleteMember($uid){
		...
		if (self::memberIdExists($uid)) {
			...
			//db::update('user_groups', 'users=users-1', "id='11'");

			//db::update('user_groups', 'users=users-1', "id='$memberInfo[groupid]'");

			//预留位置删除刷销量信息用户头像等一切关于该用户ID的信息
	...
	public static function addVip1($uid, $month, $auto){
		...
		if ($uid && $month > 0) {
			...
			if ($m = db::one('memberfields', 'money,vip,vip_expire', "uid='$uid'")) {
				...
				$vipaddmoney = common::formatMoney(cfg::get('sys', 'vipaddmoney'));
				...
				if ($money <= $m['money']) {
					...
					$moneyadd = $vip_money * $month * $vipaddmoney;

					if ($madd = db::one('members', 'parent', "id='$uid'")) {

						$uidadd=$madd['parent'];

						self::addMoney($uidadd, $moneyadd, '推荐'.$month.'个月一级会员佣金');
			...
			if ($m = db::one('memberfields', 'money,vip,vip_expire', "uid='$uid'")) {
				...
				if ($money <= $m['money']) {
					...
					if ($madd = db::one('members', 'parent', "id='$uid'")) {
						...
						self::addMoney($uidadd, $moneyadd, '推荐'.$month.'个月钻石会员佣金');
			...
			if ($m = db::one('memberfields', 'money,vip,vip_expire', "uid='$uid'")) {
				...
				if ($money <= $m['money']) {
					...
					if ($madd = db::one('members', 'parent', "id='$uid'")) {
						...
						self::addMoney($uidadd, $moneyadd, '推荐'.$month.'个月皇冠会员佣金');
	...
	public static function dayaddVip1($uid, $day, $auto){

		global $timestamp;

		if ($uid && $day > 0) {

			if ($m = db::one('memberfields', 'fabudian,vip,vip_expire', "uid='$uid'")) {

				if ($m['vip_expire']){

					$zhuijiashijian = $m['vip_expire'] ;
					...
					db::update('memberfields', array(
						...
						'vip_expire' => $zhuijiashijian + 86400 * $day,
					...
					db::update('memberfields', array(
						...
						'vip_expire' => $timestamp + 86400 * $day,
