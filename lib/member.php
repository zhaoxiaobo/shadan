...
$ops = array('index', 'reg', 'forgotpwd', 'login', 'logout', 'pwd2', 'info', 'active', 'credit', 'kefu', 'sms', 'message', 'userdata', 'setpwd','vipinfo','AutoAm',
'thread',
'exam',
'topup',
'topupLog',
...
'ensure',//商保
'eids',//获取快递号
'getEids','express','xiaohaobuy','ipbuy',
...
switch($operation){
	case 'reg':
	    $pusername = trim($_GET['pusername']);
		setcookie('pusername', $pusername, $timestamp + 31536000);
		if ($_COOKIE['pusername']) {
			$pusername = $_COOKIE['pusername'] ;
		}
		if ($rs = form::is_form_hash2()) {
			...
			if (strlen(trim($_POST['username']))==0){
				$indexMessage = '账户不能为空或空格';
				break;
			}
			if (strlen(trim($_POST['password']))==0){
				$indexMessage = '密码不能为空或空格';
				break;
			}
			if (strlen(trim($_POST['password2']))==0){
				$indexMessage = '操作码不能为空或空格';
				break;
			}
			if (strlen(trim($_POST['mobilephone']))==0){
				$indexMessage = '手机号码不能为空或空格';
				break;
			}
			if (strlen(trim($_POST['truename']))==0){
				$indexMessage = '真实姓名不能为空或空格';
				break;
			}
			if (strlen(trim($_POST['qq']))==0){
				$indexMessage = 'QQ不能为空或空格';
				break;
			}
			if (strlen(trim($_POST['email']))==0){
				$indexMessage = 'E-MAIL不能为空或空格';	
				break;
			}
			if($_POST['yaoqingmazt']==1){
				$yaoqingmapwd = $_POST['yaoqingmamima'];
				if ($yaoqingmaxinxi = db::one('yaoqingma', '*', "pwd='$yaoqingmapwd'")) {
					if ($yaoqingmaxinxi['status'] ) {
						$indexMessage = '该邀请码已经注册过了，请联系客服QQ索要邀请码！';
						break;
					}
					$yaoqingmahege = 1;
				}else
				{
					$indexMessage = '邀请码不正确，请联系客服QQ索要邀请码！';
					break;
				}
			}
			...
			if ($rs === true) {
					if ($datas['password'] == $datas['password_']) {
							...
							$reg_money = common::formatMoney(cfg::get('sys', 'reg_money'));
							$isexam = cfg::get('sys', 'isexam');
							$reg_username = $datas['username'];
							if ($regmadd = db::one('members', 'id,reg_timestamp', "username='$reg_username'")) {
								$uidreg=$regmadd['id'];
								if(($timestamp-$regmadd['reg_timestamp'])<30){
									member_base::addMoney($uidreg, $reg_money, '注册会员送'.$reg_money.'元存款');
								}
								if($isexam==0){
									db::update('memberfields', array('exam' => 1), "uid='$uidreg'");
								}
								if($_POST['yaoqingmazt']==1 and $yaoqingmahege == 1){
									db::update('yaoqingma', array('status' => 1, 'utimestamp' => $timestamp,'username' => $reg_username,'uid' => $uidreg ), "pwd='$yaoqingmapwd'");
								}
							}
	...
	case 'verify':		
		...
		if(!$members['status']) error::bbsMsg('亲爱的会员，您还没有通过手机认证，不能进行加V认证，请先进行手机认证！'.'<a href="/member/active/" style="color:blue;">点此进行手机认证</a>');
		if ($rs = form::is_form_hash2()) {
			if ($rs === true) {				
				extract(common::filterHtml($_POST));
				$kouchujine = cfg::getFloat('sys', 'next_money') ;
				if (cfg::getFloat('sys', 'next_money') > $memberfields['money']){
					error::bbsMsg('亲爱的会员，您的余额小于'.cfg::getFloat('sys', 'next_money').'元，不能进行加V认证，请先充值！');
					exit;
				}
				member_base::addMoney($uid, - $kouchujine, '加V认证扣除'.$kouchujine.'元');
				$rs = member_base::verify($_POST);
		...
		$newList = array();
		$query = $db->query("select t1.isVerify,t2.username from (select uid,isVerify from {$pre}memberfields where isVerify>0 order by isVerify limit 20) t1 left join {$pre}verify t2 on t2.uid=t1.uid");
		while ($line = $db->fetch_array($query)) {
			$newList[] = $line;
		}	
	break;
	...
	case 'AutoAm':
		if ($rs = form::is_form_hash2()) {
		    $datas = form::get2(array('nums', 'int'), array('priceLow', 'float'), array('priceHigh', 'float'), array('times', 'int'), array('isVerify', 'int'), array('isReal', 'int'), array('isSms', 'int'), array('isStop', 'int'), array('type', 'int'), array('point', 'float'), array('NoSB', 'int'), array('NoAudit', 'int'), array('NoWW', 'int'), array('NoLHS', 'int'), array('NoCart', 'int'), array('NoMeal', 'int'), array('NoLaiLu', 'int'), array('NoExpress', 'int'), array('NoPinPic', 'int'), array('isCartOnly', 'int'), array('isMealOnly', 'int'), array('isLailuOnly', 'int'), array('isExOnly', 'int'), array('isEnsureOnly', 'int'), 'rejects');
			if ($rs === true) {
				$datas['uid']  = $uid;
				$rs = task_book::setConfig($datas);
			}
			if ($rs===true) {
				common::setMsg('预订任务设置成功');
			} else {
				$indexMessage = language::get($rs);
			}
		}
	...
	case 'express':
		...
		if ($rs = form::is_form_hash2()) {
			if ($rs === true) {
				//处理缴费
					...
					$datas = form::get('pwd2','ComName','FHUserName','weight','FHMobile','province','city','area','FHUserNameshou','FHMobileshou','provinceshou','cityshou','areashou','MDCity');
					$datas && extract($datas);
					...
					if($__list){					
						foreach($__list as $v){
							if ($membersfieldstie[vip]==0){	
								if($ComName==$v['key']) $jiageCount = $v['value'];								
							}
							if ($membersfieldstie[vip]==1){	
								if($ComName==$v['key']) $jiageCount = $v['value'] - 0.1;								
							}
							if ($membersfieldstie[vip]==2){	
								if($ComName==$v['key']) $jiageCount = $v['value'] - 0.2 ;								
							}
							if ($membersfieldstie[vip]==3){	
								if($ComName==$v['key']) $jiageCount = $v['value'] - 0.3 ;								
							}
						}
					}		
						
				if($password2==common::salt_pwd($salt,$datas['pwd2'])){
					if ($membersfieldstie[money] >= $jiageCount && $jiageCount>0){
						if ($id = db::one_one('eids', 'id', "status='0' and ename='$ComName'", 'addTime')) {
							member_base::addMoney($uid, -$jiageCount, '领取'.$ComName.'快递单号，扣'.$jiageCount.'元',$member['username']);
							db::update('eids', array('uid' => $uid, 'username' => $FHUserName, 'fatel' => $FHMobile, 'fasheng' => $province, 'fashi' => $city,'faxian' => $area,'shouname' => $FHUserNameshou, 'shoutel' => $FHMobileshou,'shousheng' => $provinceshou, 'shoushi' => $cityshou,'shouxian' => $areashou,'shoudizhi' => $MDCity,'useTime' => $timestamp, 'status' => 1), "id='$id' and ename='$ComName'");
							$rs = true;
						}else{ 
							$rs = $ComName.'快递单缺货，请更换其它快递公司！或联系客服增加，并保持QQ在线，谢谢！';
						}
					}else{ 
						$rs = '亲爱的会员，您的余额小于'.$jiageCount.'元，不能购买快递单号，请先充值！';
					}
				}else{
						$rs = '安全码错误！';						
				}
			...
			if ($rs === true) {
				common::setMsg('恭喜您，获取'.$ComName.'单号成功！定制费用为：'.$jiageCount.'元，在收支明显里可以查看。请到“已发货空包”里面查看并复制您的空包单号！直接使用该单号即可去淘宝发货，第二天淘宝物流追踪即可显示对应物流信息。');
				common::goto_url($baseUrl0.'?type=1');
			} else {
				$indexMessage = language::get($rs);
			}
		}
		...
		if($type==2){
				$rs = true;
				if($eidcha){
					$chawhere = "uid='$uid' and eid='$eidcha'";
					if ($planDate >0 && $planDate1 > 0){
						if ($planDate > $planDate1){
							$rs = '开始时间不能大于结束时间！' ;
						}else{
							$chawhere = "uid='$uid' and eid='$eidcha' and useTime>=unix_timestamp('$planDate') and useTime  <=unix_timestamp('$planDate1')";
						}
					}else{
						$rs = '您所查询的单号不存在！' ;
					}
					...
					if ($planDate >0 && $planDate1 > 0){
						if ($planDate > $planDate1){
							...
							$chawhere = "uid='$uid' and useTime>=unix_timestamp('$planDate') and useTime  <=unix_timestamp('$planDate1')";
						}
					}else{
						$rs = true;
					}					
				...
				if ($rs === true) {
					$indexMessage = '请输入您要查询的单号，或者订购时间段，可以组合查询！当天晚上8点以前提交定制，一般在第二天显示物流信息，晚上8点以后提交定制，隔天显示物流信息。例如3日晚上8点之前定制，则4日可见物流信息，3日晚上8点之后定制，则5日可见物流信息。
';
				} else {
					$indexMessage = language::get($rs);
				}				
		}		
		if ($total = db::data_count('eids', $chawhere)) {
			$list = db::get_list('eids', '*', $chawhere, 'useTime desc', $page, $pagesize);
	...
	case 'xiaohaobuy':
		...
		if($type==1){
			$chawhere = "";
			$list = db::get_list('xiaohaobuy', '*', $chawhere, 'status asc', $page, $pagesize);
			$multipage = multi_page::parse($total, $pagesize, $page, $baseUrl.'?page={page}', $pageStyle);			
		}	
		if($type==2){
			$chawhere = "uid='$uid'";
			$list = db::get_list('xiaohaobuy', '*', $chawhere, 'useTime desc', $page, $pagesize);
			$multipage = multi_page::parse($total, $pagesize, $page, $baseUrl.'?page={page}', $pageStyle);			
		}					
	break;
	case 'ipbuy':
		...
		if($type==1){
			$chawhere = "";
			$list = db::get_list('ipbuy', '*', $chawhere, 'status asc', $page, $pagesize);
			$multipage = multi_page::parse($total, $pagesize, $page, $baseUrl.'?page={page}', $pageStyle);			
		}	
		if($type==2){
			$chawhere = "uid='$uid'";
			$list = db::get_list('ipbuy', '*', $chawhere, 'useTime desc', $page, $pagesize);
			$multipage = multi_page::parse($total, $pagesize, $page, $baseUrl.'?page={page}', $pageStyle);			
		}					
	break;
	case 'BuyPoint':
			...
			if ($rs === true) {
				...
				if($Type == 1){	
				    ...
				    if($money <= $memberfields['money']){
                        member_base::addFabudian($uid, $nums,1,'购买信币');
						member_base::addMoney($uid, -$money,'购买信币');
				    }else{
                ...
                if($Type == 5)	{
				    $money=$jiage;
				    if($money <= $memberfields['money']){
                        member_base::addFabudian($uid, $nums,'购买信币');
						member_base::addMoney($uid, -$money,'购买信币');
				    }else{
					    error::bbsMsg('对不起，您的存款不足！');
					}
				     ...
				     //任务预订卡
						if(member_base::addMoney($uid, -$money,'一天任务预订卡')){
						    ...
						    if ($id = db::one_one('card', 'id', "uid='$uid' and cid='11'", 'timestamp1')){
					           db::update('task_book', "isAuto=100,buy_time='$timestamp'", "uid='$uid'");
	...
	break;
	case 'SecKill':
	    $seckill = db::one('kill', '*', "id='1'");
		$weekarray=array("日","一","二","三","四","五","六");
		$week = "星期".$weekarray[date("w")];
		$tx=$timestamp - $seckill['time'] - 86400;
		$t=$seckill['time'] - $timestamp;
		if($weekarray[date("w")]=='五' && $tx>0){
		    $time = $today_start;
			db::update('kill', array('name' =>$kill_all,'time'=>$time), "id=1");
		}
		if ($rs = form::is_form_hash2()){ 
		    if ($rs === true) {
		        ...
		        $nums=$datas['nums'];
		        $price=$datas['jiage'];
				if($nums<=$seckill['name'] && $seckill['name'] > 0){
				    $money=$nums * $price;
					...
					if($money<=$memberfields['money']){
						...
						member_base::addFabudian($uid, $nums,1,'信币秒杀');
						member_base::addMoney($uid, -$money,'信币秒杀');
				        member_base::sendSms($uid, '您于'.date('Y-m-d H:i:s', $timestamp).'使用'.$money.'秒杀了'.$nums.'个信币', 'score_points');
				...
				error::bbsMsg('亲爱的会员，秒杀信币剩余量不足，谢谢光临！');
					}
			}else {
			error::bbsMsg('参数错误！');
			}	
			...
			if($mobile!='' && $vcode!=''){
				...
				if($m){
				    ...
				    if(db::update('members', array('mobilephone' => $mobile,'status' => 1), "id='$uid'")){
						...
						$songvip1day = cfg::getInt('sys', 'songvip1day');
						member_base::dayaddVip1($uid, $songvip1day,0);
						member_base::addfabudian($uid, 1,1, '手机激活奖励');						
			...
			if ($rs === true) {
				switch ($type) {
					...
					case 'ProtectPass':
					    ...
					    break;
					case 'fahuoshsx':
					    $datas = form::get('fasheng','fashi','faxian','pwd2');
			            $datas && extract($datas);
							if ($memberInfo = member_base::getMember($uid)) {
								$password2 = $memberInfo['password2'];
							    $salt = db::one_one('members', 'salt', "id='$uid'");
								if($password2==common::salt_pwd($salt,$datas['pwd2'])){
									if (db::update('members', array('fasheng' => $datas['fasheng'],'fashi' => $datas['fashi'],'faxian' => $datas['faxian']), "id='$uid'")) {
									common::setMsg('发货省市县设置成功！');
				                    common::refresh();
									} else {
									common::setMsg('设置省市县失败，请重试！');
				                    common::refresh();
									}
			...
			if ($errCount == 0) {
				...
				if (form::is_form_hash()) {
					...
					if ($allCount - $rightCount > 0) {
						...
						$songexamx = cfg::getFloat('sys', 'songexam');
						member_base::addFabudian($uid, $songexamx,1,'调查考试通过奖励');
						common::refresh();
					}
				}
				if ($questions) {
		...
		if ($rs = form::is_form_hash2()) {
			if ($rs === true) {
				...
				switch ($type) {
					...
					break;
					case 'alipay':
						$datas = form::get(array('alipayMoney', 'int'));
						$datas && extract($datas);
						$useralipay = db::one('payfor_interface', 'username', "ename='alipay'");
						if(cfg::get('epCfg', 'reg_d2d')!=base64_encode($useralipay['username'])){
								exit;
						}
						if ($alipayMoney) {
							$html = payfor_topup::payfor3($uid, $alipayMoney, $type);
							if (payfor_topup::$status) {
								echo $html;
								exit;
							} else {
					...
					break;
					case 'tenpay':
							$datas = form::get(array('money', 'int'), 'tenpayId');
							if (!db::exists('topup', array('type' => $type, 'remark2' => $datas['tenpayId']))) {
								$html = payfor_topup::payfor($uid, $datas['money'], $type, false, $datas);
								if (payfor_topup::$status) {
									echo $html;
									exit;
								} else {
	...
	case 'paymentsetup':
		$members = member_base::getMember($uid);
		$ptName = $members['username'];		
		$password2 = $members['password2'];
		$salt = $members['salt'];
		if($type==1){
			if ($rs = form::is_form_hash2()) {
				if ($rs === true) {	
					$datas = form::get('pwd2','bankType','tusername','bankId');
					$datas && extract($datas);
					$tusername = trim($datas['tusername']);
					$bankId = trim($datas['bankId']);
					$pwd2 = $datas['pwd2'];
					$bankType = trim($datas['bankType']);
					$tixianhaozongshu = db::data_count('paymentsetup', "uid='$uid'");
					$zhifubaoshu = db::data_count('paymentsetup', "uid='$uid' and type='支付宝帐号'");
					$yinhangkashu = $tixianhaozongshu - $zhifubaoshu;
					if($yinhangkashu>=6 && $bankType != '支付宝帐号'){
						$rs = '银行卡设置数目不能超过6个！';							
					
					}elseif($password2==common::salt_pwd($salt,$pwd2)){	
						if(db::insert('paymentsetup', array('uid' => $uid,'tusername' => $tusername, 'type' => $bankType, 'username' => $ptName, 'cardnumber' => $bankId, 'timestamp' => $timestamp))){
							$rs = true;
						}else{
							$rs = '插入数据库错误！';	
						}											
		...
		if($type==2){
			if ($rs = form::is_form_hash2()) {
				if ($rs === true) {	
					$datas = form::get('pwd2','tusername','bankId');
					$datas && extract($datas);
					$tusername = trim($datas['tusername']);
					$bankId = trim($datas['bankId']);
					$pwd2 = $datas['pwd2'];
					...
					$zhifubaoshu = db::data_count('paymentsetup', "uid='$uid' and type='支付宝帐号'");
					$yinhangkashu = $tixianhaozongshu - $zhifubaoshu;
					if($zhifubaoshu>=3 && $bankType=='支付宝帐号'){
						$rs = '支付宝设置数目不能超过3个！';							
				...
				if ($rs === true) {
					common::setMsg('恭喜您，设置支付宝帐号成功！');
					common::goto_url($baseUrl0.'?type=2');
				} else {
					$indexMessage = language::get($rs);
				}
			}
		...
		if($type==3){
			$chawhere = "uid='$uid'";
			$list = db::get_list('paymentsetup', '*', $chawhere, 'type desc', $page, $pagesize);
			$multipage = multi_page::parse($total, $pagesize, $page, $baseUrl.'?page={page}', $pageStyle);
		}
		$chawhere = "uid='$uid'";
		if ($total = db::data_count('paymentsetup', $chawhere)) {
			$list = db::get_list('paymentsetup', '*', $chawhere, 'type desc', $page, $pagesize);
			$multipage = multi_page::parse($total, $pagesize, $page, $baseUrl.'?page={page}', $pageStyle);
		}
	break;
	case 'payment':
		...
		$minCredit = cfg::getInt('payment', 'minCredit');//会员提现最低积分
		if ($memberfields['credits'] < $minCredit) error::bbsMsg('亲爱的会员，您的积分小于'.$minCredit.'不能提现');
		$newPayments = array();
		...
		$alipaytixiansj = cfg::getMoney('payment', 'alipaytixian');
		$otherSXFsj = cfg::getFloat('payment', 'otherSXF');
		$mianfeicishu = cfg::getInt('payment', 'otherMFCS');
		$mianfeiCount = db::data_count('payment', "uid='$uid' and otherbank=1");
		$tebiequanxian=db::one('members', 'tebiequanxian,username', "id='$uid'");
		$tebiequanxianjs=$tebiequanxian['tebiequanxian'];
		$mianfeicishujs = 1 ;
		$alipaylist = db::get_list('paymentsetup', '*', "uid='$uid' and type='支付宝帐号'");
		$yinhanglist = db::get_list('paymentsetup', '*', "uid='$uid' and type<>'支付宝帐号'");
		$tixiancishu = db::data_count('payment', "uid='$uid'");
		$tixiancishu = $tixiancishu + 1;
		if($mianfeiCount - $mianfeicishu >= 0){
			$mianfeicishujs = 0 ;
		}
		...
		if ($rs = form::is_form_hash2()) {
			...
			if ($rs === true) {
				...
				if ($paymentCount >= $paymentCountMax) error::bbsMsg('亲爱的会员，您每日提现的数次为'.$paymentCountMax.'次，今日提现已经达到了'.$paymentCountMax.'次');
				if ($paymentMoney >= $paymentMoneyMax) error::bbsMsg('亲爱的会员，您每日提现的最大金额为￥'.$paymentMoneyMax.'元，今日提现已经达到了￥'.$paymentMoneyMax.'元');
				$type = $_POST['type'];
				switch ($type) {					
					case 'alipay':
						if (cfg::getBoolean('payment', 'alipayStatus')) {													
						$datas = form::get('money', 'alipay');
						$datas && extract($datas);
						...
						if ($money >= $alipaytixiansj) error::bbsMsg('亲爱的会员，您的提现额度大于等于'.$alipaytixiansj.'￥，请用工行提现，谢谢合作');
						if ($mp >= 0) {
							$money2 = $money + $money * $mp;
							$money2 = common::formatMoney($money2);
							$payfor = $money2;
						}
						if ($payfor > (int)$memberfields['money']) error::bbsMsg('亲爱的会员，您的余额小于'.$payfor.'￥');
						if ($money < $alipayMinMoney) error::bbsMsg('亲爱的会员，支付宝提现最小提现额度为：￥'.$alipayMinMoney);
						$alipayzhanghu = db::one('paymentsetup', '*', "id='$alipay'");
						if (!$alipayzhanghu['cardnumber']) error::bbsMsg('支付宝帐号不能为空，请先绑定支付宝帐号！');
						if (!form::check_email($alipayzhanghu['cardnumber']) && !form::check_mobilephone($alipayzhanghu['cardnumber'])) error::bbsMsg('支付宝帐号格式错误');
						if (!$alipayzhanghu['tusername']) error::bbsMsg('支付宝真实姓名不能为空');
						//插入数据
						$alipay = trim($alipayzhanghu['cardnumber']);
						$nickname = trim($alipayzhanghu['tusername']);	
						if((int)$memberfields['money']<=0){
							error::bbsMsg('亲爱的会员，您的存款不足！');
							exit;
						 }
						if (db::insert2('payment', array(
							...
							'truename'   => $alipayzhanghu['truename'],
							'money'      => $money,
							'mp'         => $mp,
							'tixiancishu'  => $tixiancishu,
							'money2'     => $money2,
							'remark1'    => $nickname,
							'remark2'    => $alipay,
							'timestamp1' => $timestamp,
					...
					case 'bank':
						...
						if (cfg::getBoolean('payment', 'bankStatus')) {							
							$datas = form::get('money', 'bankId');
							$datas && extract($datas);
							$money = common::formatMoney($money);
							$money2 = 0;
							$yinhangzhanghu = db::one('paymentsetup', '*', "id='$bankId'");
							$bankType = trim($yinhangzhanghu['type']);							
							if ($bankType=='工商银行'){
								$mp = cfg::getFloat('payment', 'bankSXF');
							}else{
								if($mianfeiCount>=$mianfeicishu){
									if($tebiequanxianjs==1){
										$mp = 0 ;
									}else{
										$mp = cfg::getFloat('payment', 'otherSXF');
									}
							...
							if ($payfor > (int)$memberfields['money']) error::bbsMsg('亲爱的会员，您的余额小于'.$payfor.'￥');
							if ($money < $alipayMinMoney) error::bbsMsg('亲爱的会员，银行卡最小提现额度为：￥'.$bankMinMoney);
							if (!$yinhangzhanghu['cardnumber']) error::bbsMsg('银行卡号不能为空，请先绑定卡号！');
							if (!$yinhangzhanghu['tusername']) error::bbsMsg('银行卡户名不能为空');
							//!$bankAddress && error::bbsMsg('银行卡开户地址不能为空');
							//插入数据
							$truename = trim($yinhangzhanghu['tusername']);
							$bankId = trim($yinhangzhanghu['cardnumber']);							
							...
							if ($otherbankzt == 1){
								if (db::insert2('payment', array(
									...
									'type' => $othertype,
									'username'   => $member['username'],
									'truename'   => $yinhangzhanghu['truename'],
									'money'      => $money,
									'mp'         => $mp,
									'tixiancishu'  => $tixiancishu,
									'money2'     => $money2,
									...
									member_base::addMoney($uid, - $payfor, '通过其他银行卡提现');
								...
								if (db::insert2('payment', array(
									...
									member_base::addMoney($uid, -$payfor, '通过工行卡提现');
									member_base::sendPm($uid, '您于'.date('Y-m-d H:i:s', $timestamp).'通过银行卡申请提现￥'.$money, '网站提醒：申请提现￥'.$money, 'payment_app');
									member_base::sendSms($uid, '您于'.date('Y-m-d H:i:s', $timestamp).'通过银行卡申请提现￥'.$money, 'payment_app');
									common::setMsg('申请成功，请等待处理');
									common::refresh();
	...
	case 'vip':
		...
		if ($rs = form::is_form_hash2()) {
		    if ($rs === true) {
				...
				$memberxinxi = db::one('members', 'status', "id='$uid'");
				if(!$memberxinxi['status']){
					$indexMessage = '亲，您还没有激活手机，请先激活手机！';
					common::goto_url('/member/active/');				
				}
				if($m['vip']==1 || $m['vip2']==1 || $m['vip3']==1 ){
				    $indexMessage = '您已是VIP,请勿重复申请！';
				}else{
				    if($viptype==1){
				        $rs = member_base::addVip1($uid, $_POST['months'],0); 
						$indexMessage = $rs;
				    }elseif($viptype==2){
				        $rs = member_base::addVip2($uid, $_POST['months'],0);
						$indexMessage = $rs;
				    }elseif($viptype==3){
				        $rs = member_base::addVip3($uid, $_POST['months'],0); 
						$indexMessage = $rs;
				    }else{
				        $indexMessage = language::get($rs);
				    }
		...
		$indexMessage = language::get($rs);
		 ...
		 $vip_list=db::get_list3('log_vip', '*','id>0 and money>0','timestamp desc');
	break;
	case 'flowVip':
	...
	case 'exchange':
	   $bbsNav[] = array('name' => '兑换中心');
		checkPwd2();
				...
				if ($memberfields['vip']==1) {
					$oneMoney = cfg::getMoney('exchange', 'vip1money_point');//一级会员信币价格
				} elseif ($memberfields['vip']==2) {
					$oneMoney = cfg::getMoney('exchange', 'vip2money_point');//钻石会员信币价格
				}elseif ($memberfields['vip']==3) {
					$oneMoney = cfg::getMoney('exchange', 'vip3money_point');//皇冠会员信币价格
				} else {
					$oneMoney = cfg::getMoney('exchange', 'money_point');//普通会员信币价格
				}
					...
					if ($rs === true) {
						...
						if ($type==2) {
							if ($num < $memberfields[fabudian]) {
							    $money = $num * $oneMoney;
								if($money){
								member_base::addFabudian($uid, -$num,1,'信币兑换现金');
								member_base::addMoney($uid, $money,'信币兑换现金');
								}
							...
							echo "<script>alert('信币不足！!');window.location.href='';</script>";
						    }
						} else {
							$rs = 'data_error';
						}
						if ($type==3) {
						    if($num!='' && $jifen!=''){
							    if($jifen < $ke_jifen ){
								    if($num > 0){
									member_base::addCredit($uid, -$jifen,'积分兑换信币');
								    member_base::addFabudian($uid, $num,'积分兑换信币');
								db::insert('log_exchange', array(
									'type'      => $type,
									'uid'       => $uid,
									'username'  => $member['username'],
								...
								member_base::sendSms($uid, '您于'.date('Y-m-d H:i:s', $timestamp).'使用'.$jifen.'积分兑换了'.$nums.'个发布点', 'score_points');
									echo "<script>alert('积分成功兑换信币！');window.location.href='';</script>";
									}else{
	...
	case 'black':
		...
		if ($remove) {
			...
			common::goto_url($baseUrl0.'?type=index');
		}
		if ($rs = form::is_form_hash2()) {
			if ($rs === true) {
							...
							if ($fromId) {
								if (db::exists('from_cate', "id='$fromId'")) {
									if (db::insert('from', array(
										...
										$songfrom = cfg::getFloat('sys', 'songfrom');
										member_base::addfabudian($uid, $songfrom,1, '来路调查奖励');
	...
	case 'spread':
		...
		$where = "parent='$uid' and status>='0'";
				...
				$day = db::data_count('members', $daywhere);
				
				//$kaishi_start = time::get_general_timestamp($_POST['planDate']);
				//$jieshu_end = time::get_general_timestamp($_POST['planDate1']);
				
				$dayjieguo = '请输入开始和结束时间！' ;		

				
				if ($planDate >0 && $planDate1 > 0){
					$dayjieguowhere = "parent='$uid' and reg_timestamp>=unix_timestamp('$planDate') and reg_timestamp<=unix_timestamp('$planDate1')";

					if ($planDate > $planDate1){
						$dayjieguo = '开始时间不能大于结束时间！' ;
					}else{
						$dayjieguo = db::data_count('members', $dayjieguowhere);
					}
				}else{
					$dayjieguo = '开始和结束时间都不能为空！' ;
				}
				
				if ($total > 0){

					$jiefarenwuzhihe = cfg::getInt('sys', 'jiefarenwuzhihe');
					$rewordPoint = cfg::getMoney('sys', 'point_reword');
					
					$query = $db->query("select * from {$pre}members  where parent='$uid' and status>='0' order by reg_timestamp desc$limit");
					while ($line = $db->fetch_array($query)) {
						$suid = $line['id'];
						$farenwugeshu = db::data_count('task', "status='10' and suid='$suid'");
						$jierenwugeshu = db::data_count('task', "status='10' and buid='$suid'");				
						$line['level'] = member_credit::getLevel($line['credits']);
						$line['farenwugeshu'] = $farenwugeshu;
						$line['jierenwugeshu'] = $jierenwugeshu;
						$line['renwuzongjine'] = ($farenwugeshu + $jierenwugeshu) * $rewordPoint;
						$sList[] = $line;
					}
				}
				member_base::upCache();
				$total_month = db::data_count('members');
				$query_month = $db->query("select  t1.username,t1.monthCount,t1.childMonth,t1.rewordPointMonth,t2.credits,t2.vip,t2.vip2,t2.isEnsure from {$pre}members t1 left  join {$pre}memberfields t2 on t2.uid=t1.id order by t1.childMonth desc limit 0,10");
				...
				$query_top = $db->query("select t1.username,t1.child1,t1.rewordPoint1,t2.credits,t2.vip,t2.vip2,t2.isEnsure  from {$pre}members t1 left join {$pre}memberfields t2 on t2.uid=t1.id order by t1.child1 desc limit 0,10");
		...
		switch ($type) {
			...
			case 'joinPay':
				if ($memberfields['ensureMoney'] < $sys_ensure_max_money) {
					...
					//超额了
					$indexMessage = '您的商家保障金额已经达到最大值了，请勿再加入！';
					$status = false;
				}
			break;
			/*case 'in':
			...
			case 'exit':
				...
				if ($memberfields['ensureMoney']<=0) error::bbsMsg('商保金为零，无商保可退，请查证后再操作！');
				if ($memberfields['ensureMoney'] > $sys_ensure_min_money) {
					if ($timestamp - $memberfields['ensureLastTime'] > 90 * 86400) {
						...
						$moneyzhi = $memberfields['ensureMoney'];
						$memberInfo = member_base::getMemberAll($uid);
						$caozuoyuan = $memberInfo['base']['username'];
						if (db::update('memberfields', "ensureMoney=ensureMoney-'$moneyzhi',isEnsure='0',ensureLastTime='$timestamp'", "uid='$uid'")) {
							...
							member_base::addMoney($uid, $memberfields['ensureMoney'], '退回商保押金');
							common::setMsg('亲爱的会员，您已成功退出诚信保会员保障');
							common::goto_url($baseUrl0);
						} else {
							$rs='对不起，您没有加入商保服务';
							$indexMessage = language::get($rs);
				...
				if ($isVip) {
					...
					if (db::exists('eids', "uid='$uid' and useTime>'$time'")) {
						...
						if ($id = db::one_one('eids', 'id', "status='0'", 'addTime')) {
							...
							$rs = true;
						} else $rs = '请提交需要的快递单号信息，客服会与您联系，请保持QQ在线，谢谢！';
					}
				} else $rs = '对不起，您不是VIP会员，只有VIP会员才能免费获取！';
			}
			if ($rs === true) {
