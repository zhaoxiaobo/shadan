...
switch ($m) {
	case 'index':
		...
		if ($total = task_buyer::total($taskId, $status2)) {
					$isreal = db::data_count('buyers', "type='2' and uid='$uid' and isreal='1' and status='0'");
					$real = db::data_count('buyers', "type='2' and uid='$uid' and isreal='0' and status='0'");
					$bList = task_buyer::getList(2, $uid, $status2);
...
case 'CreateLaiLuMission':
		...
		if ($rs = form::is_form_hash2()) {
			$datas = form::get('nickname', 'itemurl', 'visitTip', 'visitKey', array('visitWay', 'int'), array('price', 'float'), array('isPriceFit', 'int'), array('pointExt', 'float'), array('times', 'int'),  array('scores', 'int'), array('isRemark', 'int'), 'remark', array('isShare', 'int'),array('isAddress', 'int'), 'address',array('share', 'int'), 'tips',array('isVerify', 'int'),array('isLimit', 'int'), array('limit', 'int'), array('chssp', 'int'),array('isNoword', 'int'),
				...
				'qq','cbxAddress',array('expressTM', 'int'),
				array('isEnsure', 'int'),//是否商保
				array('ispinimage', 'int'),
				array('ensurePoint', 'float'),
				array('isScore', 'int'), array('scoreLvl', 'int'),array('iscomplain', 'int'), array('complain', 'int'), array('isCredit', 'int'), array('creditLvl', 'int'), array('isGood', 'int'), array('goodLvl', 'int'), array('isBlack', 'int'), array('blackLvl', 'int'), array('isFame', 'int'), array('fameLvl', 'int'), array('isPlan', 'int'),'photourl','planDate');
			if ($isHot) $count = (int)$_POST['count'];
			else $count = 1;
			if ($rs === true) {
				$rs = task_pai::add($datas, $uid, $count ,'photourl');
			}
			if ($rs === true) {
				//保存模板
				$datas2 = form::get(array('isTpl', 'int'), 'tplTo');
