...
");
    ...
    //登录验证
		...
		//我是接手方
		$bbsList = array();
		$query = $db->query("select title,id,timestamp from {$pre}threads where fid =5 ORDER BY timestamp desc limit 0,4");
		while ($line = $db->fetch_array($query)) {
			$bbsList[] = $line;
		}
		//我是发布方
		$faList = array();
		$query = $db->query("select title,id,timestamp from {$pre}threads where fid =4 ORDER BY timestamp desc limit 0,4");
		while ($line = $db->fetch_array($query)) {
			$faList[] = $line;
		}
		//掌柜经验
		$jyList = array();
		$query = $db->query("select title,id,timestamp from {$pre}threads where fid =9 ORDER BY timestamp desc limit 0,4");
		while ($line = $db->fetch_array($query)) {
			$jyList[] = $line;
		}
		//问题建议
		$askList = array();
		$query = $db->query("select title,id,timestamp from {$pre}threads where fid =6 ORDER BY timestamp desc limit 0,4");
		while ($line = $db->fetch_array($query)) {
			$askList[] = $line;
		}
// THE END
