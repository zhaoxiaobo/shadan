...
switch($operation){
	...
	case 'threadlist':
		...
		if(bbs_forums::forumExists($fid) || ($fid = bbs_forums::getForumId($ename))){
			...
			$keywords    = '刷销量,刷信用,互刷销量平台,刷收藏,刷流量,{webName2}';
	...
	case 'thread':
		...
		$thread = bbs_thread::getThread($tid);
		$fid = $thread['fid'];
		$tiezix = db::one_one('posts', 'id', "tid='$tid' and first='1'");
		...
		$keywords = '刷销量,刷信用,互刷销量平台,刷收藏,刷流量,'.$web_name2;
	...
	case 'edit':
		...
		if ($post = bbs_thread::getPost($pid, false)) {
			if ($post['first']) {
				...
				if (!bbs_thread::isThreadAuthor($thread['id'], $uid)) error::bbsMsg('很抱歉，您没有权限编辑该主题');
				$editType = 'thread';
