...
$ops = array('memberTipIndex', 'login','review', 'active', 'sendmsg','sendExpress', 'sendsms', 'viewmsg', 'pwd2', 'tip', 'upload', 'card','autotk','membertishi','buytaobao','buyip',
'activeSeller', 'sellerTruth', 'sellerAddress', 'setTaskQuery', 'buyerPsw', 'buyerAddress', 'chooseBuyer', 'showTaskTip', 
'taskVisitWay', 'taskInfo', 'confirmExpress', 'grade', 'complain', 'joinClub', 'taskBook', 'taskComment',  'ensure',
'reflow', 'buyerInfo', 'buyerLevel', 'tyro','zuijiayongjin','chakanverify','taskjietu','taskchakan','xiugaitixian','deltixian'
);
...
switch ($operation) {
	...
	case 'chakanverify':
		if ($sid) {
			$task = db::one('verify', '*', "id='$sid'");
			$taskpinimage = string::ubbDecode($task['fileroad']);
		} else {
			showmessage('参数错误');
		}
	break;
	case 'xiugaitixian':
		if ($sid && $uid) {
			...
			$salt = $members['salt'];
			if ($paymentsetup = db::one('paymentsetup', '*', "id='$sid'")) {
				if ($rs = form::is_form_hash2()) {
					...
					$pwd2 = $datas['pwd2'];											
					if($password2==common::salt_pwd($salt,$pwd2)){
						if(db::update('paymentsetup', array('tusername' => $tusername, 'cardnumber' => $bankId, 'timestamp' => $timestamp),"id='$sid'")){
							showmessage('更新帐号成功！');
						}else{
							showmessage('更新数据库错误！');
						}	
	...
	case 'deltixian':
		if ($sid && $uid) {
			...
			$password2 = $members['password2'];
			$salt = $members['salt'];
			if ($paymentsetup = db::one('paymentsetup', '*', "id='$sid' and uid='$uid'")) {
				if ($rs = form::is_form_hash2()) {
					$datas = form::get('pwd2','shifoutijiao');
					$datas && extract($datas);
					$shifoutijiao = trim($datas['shifoutijiao']);
					if($shifoutijiao==0){
						showmessage('已取消删除！');
						exit;
					}					
					$pwd2 = $datas['pwd2'];											
					if($password2==common::salt_pwd($salt,$pwd2)){
						if(db::delete('paymentsetup', "id='$sid' and uid='$uid'")){
							showmessage('删除帐号成功！');
						}else{
							showmessage('删除时数据库错误！');
						}	
					} else {
						showmessage('操作码错误');
					}
	...
	case 'taskjietu':
		if ($sid) {
			if ($task = db::one('task', '*', "id='$sid'")) {				
				if ($task['status']<3){
				 	showmessage('当前任务状态不需要上传截图！');
					exit;
				}
				if ($task['status']>9){
				 	showmessage('任务已完成或已锁定，不能再提交了！');
					exit;
				}
				$tudizhi = explode('" class="preview">',$task['shchjietu']);
				$tuchangdu = count($tudizhi);				
				if($tuchangdu>10){
				 	showmessage('提交图片超过10张，不能再提交了！');
					exit;
				}
				if ($task['buid'] == $uid) {
					if ($rs = form::is_form_hash2()) {
						if ($rs === true) {
							$datas = form::get('remark');
							$datas && extract($datas);
							$remark = $datas['remark'];																		
							if ($rs === true) {
								$remark = $task['shchjietu'].$remark;
								db::update('task', array('shchjietu' => $remark), "id='$sid'");
								$_showmessage = '上传截图已经完成，请进行下一步操作！';								
							} else {
	...
	case 'taskchakan':
		if ($sid) {
			...
			if ($task['buid'] != $uid && $task['suid'] != $uid){
				showmessage('很抱歉，该任务不是您接手的或不是你发布的，您无权查看！');
			}else{
				$tudizhi = explode('" class="preview">',$task['shchjietu']);
				$tuchangdu = count($tudizhi);
				if($tuchangdu>10) $tuchangdu = 10;
				
				for($inum = 0; $inum < $tuchangdu; $inum++) {	
					$tudizhifen = str_replace('<img src="','',$tudizhi[$inum]);
					$tudizhiyou = substr ($tudizhifen, -3);
					$tudizhizong=$tudizhizong.'<img src="'.$tudizhifen.'"><br>';
				}
			}
		} else {
			showmessage('参数错误');
	...
	case 'buytaobao':
		if ($sid && $uid) {
			if ($xiaohaobuy = db::one('xiaohaobuy', '*', "id='$sid'")) {
				$zongjine = $xiaohaobuy['danjia'];
				$memberfields = member_base::getMemberFields($uid);	
				$status = $xiaohaobuy['status'];			
				if ($rs = form::is_form_hash2()) {
					if($status==1){
						$_showmessage = '该小号已经被购买，请重新选择！';
					}else{
						if($memberfields['money'] > 0 && $zongjine < $memberfields['money'] && $zongjine > 0){
							$usernamex = db::one('members', 'username', "id='$uid'");
							$username = $usernamex['username'];
							member_base::addMoney($uid, -$zongjine, '购买淘宝小号');
							db::update('xiaohaobuy', array('status' => 1,'uid' => $uid,'username' => $username,'useTime' => $timestamp), "id='$sid'");
							$_showmessage = '购买成功，请到我购买的小号里查看！';
						}else{
							$_showmessage = '您的存款不足以购买该小号！';
						}
					}
				}				
			} else {
				showmessage('对不起，不存在该小号');
			}
		} else {
			showmessage('参数错误');
		}
	...
	case 'zuijiayongjin':
		if ($sid && $uid) {
			$members = member_base::getMember($uid);
			$membersuser = $members['username'];
			$renwuxinxi = db::one('memberfields', 'fabudian', "uid='$uid'");
			$renwufabudian = $renwuxinxi['fabudian'];

			if($renwufabudian<=0){
				showmessage('信币不足，请购买信币，追加失败！');
				break;
			}
			$password2 = $members['password2'];
			$salt = $members['salt'];
			if ($task = db::one('task', '*', "id='$sid'")) {
				$taskzhuijiageshu = (int)$task['pointExt'];
				if ($rs = form::is_form_hash2()) {
					$datas = form::get('pwd2','pointExtsj');
					$datas && extract($datas);					
					$pointExtsj = trim($datas['pointExtsj']);
					$pwd2 = $datas['pwd2'];	
					if($pointExtsj<=0){
						showmessage('追加信币个数小于等于0，追加失败！');
						break;
					}
					if($pointExtsj>$renwufabudian){
						showmessage('您的信币余额小于追加信币个数，请购物信币，追加失败！');
						break;
					}
					if($task['status']>=10){
						showmessage('您的任务已完成或者任务已锁定，追加失败！');
						break;
					}
					if($password2==common::salt_pwd($salt,$pwd2)){						
						if($task['status']<10 && $pointExtsj<=$renwufabudian && $pointExtsj>0){
							$pointExtzhuijia = $task['pointExt'] + $pointExtsj;
							...
							if(db::update('task', array('pointExt' => $pointExtzhuijia, 'point' => $zongpoint), "id='$sid'")){
								member_base::addFabudian($uid, -$pointExtsj, 1, $zhuijiaqu,$membersuser);
								showmessage('追加信币成功！');
							}else{
								showmessage('更新数据库错误！');
							}
						}else {
							showmessage('任务已完成或任务已锁定或追加信币个数小于0或信币余额不足，追加失败！');
						}
					} else {
						showmessage('操作码错误');
					}
	...
	case 'buyip':
		if ($sid && $uid) {
			if ($ipbuy = db::one('ipbuy', '*', "id='$sid'")) {
				$zongjine = $ipbuy['danjia'];
				$memberfields = member_base::getMemberFields($uid);	
				$status = $ipbuy['status'];			
				if ($rs = form::is_form_hash2()) {
					if($status==1){
						$_showmessage = '该卡密已经被购买，请重新选择！';
					}else{
						if($memberfields['money'] > 0 && $zongjine < $memberfields['money'] && $zongjine > 0){
							$usernamex = db::one('members', 'username', "id='$uid'");
							$username = $usernamex['username'];
							member_base::addMoney($uid, -$zongjine, '购买IP卡密');
							db::update('ipbuy', array('status' => 1,'uid' => $uid,'username' => $username,'useTime' => $timestamp), "id='$sid'");
							$_showmessage = '购买成功，请到我购买的卡密里查看！';
						}else{
							$_showmessage = '您的存款不足以购买该IP卡密！';
						}
					}
				}				
			} else {
				showmessage('对不起，不存在该IP卡密');
			}
		} else {
			showmessage('参数错误');
		}
	...
	case 'taskInfo':
		if ($sid) {
			if ($task = task_base::_get($sid)) {
				if ($task['status'] < 1) showmessage('错误');
				if ($task['buid'] == $uid ||$task['suid']==$uid) {
	...
	break;
		...
		if ($sid) {
			if ($task = task_base::_get($sid)) {
				if ($task['status'] < 8) showmessage('错误');
				if ($task['suid'] == $uid) {
					$isBuyer = true;
				}else {
					showmessage('错误，您不能追评该任务');
				}
				$grade = task_grade::get($sid, $uid);
				if (!$grade) {
					showmessage('对不起,请您先评价完毕后再追评！');
				}
				if ($rs = form::is_form_hash2()){
				    if(1 <= $memberfields['fabudian']){
					    if ($rs === true) {
						     $datas = form::get('review');
						     $datas = common::filterHtml($datas);
							 member_base::addFabudian($uid, -1,'追加评论');
						     $rs = task_grade::review($task['id'], $uid, $datas['review'], 0);
					    }
					    if ( $rs > 0) {
						     showmessage('追评成功');
					    } else {
	...
	case 'sendExpress':
		...
		if ($sid) {
			if ($task = task_base::_get($sid)) {
				if ($task['status'] != 6) showmessage('错误');
              //  print_r($task);
				if ($rs = form::is_form_hash2()) {
					$expressNum = $_POST['expressNum'];
					$rs=db::update('task', array('expressNum' => $expressNum,'status' => 7), "suid='$uid' and id='$sid'");
					if ($rs === true) {
						showmessage('确认成功');
					} else {
						$indexMessage = language::get($rs);
