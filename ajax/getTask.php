...
foreach ((array)$$task_danbao as $k => $v){
...
if ($fromInstation && $au >= 8 && $type >= 1 && $type <= 4) {
	...
	if ($total = db::data_count('task', "type='$type' and status in('1','2','3','4','5','6','7','8','9')")) {
		...
		if($jiage==11){	
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,stimestamp desc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");
		}elseif($jiage==12){
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,stimestamp asc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");
		}elseif($jiage==13){
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,price desc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");
	
		}elseif($jiage==14){
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,price asc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");	
		}elseif($jiage==15){
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,times desc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");
		}elseif($jiage==16){
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,times asc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");	
		}elseif($jiage==17){
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,point desc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");	
		}elseif($jiage==18){
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,point asc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");	
		}elseif($stype==3){
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where id='$renwuid'  and  type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,point asc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");	
		}else{
			$query = $db->query("select t1.*,t2.credits,t2.vip,t1.isEnsure from (select * from {$pre}task where type='$type' and status in ('1','2','3','4','5','6','7','8','9') order by status,svip desc,pointExt,point desc,stimestamp desc$limit) t1 left join {$pre}memberfields t2 on t2.uid=t1.suid");
		}
	...
	if($stype==1){
	   if($line['times']==0){
	        ...
	        if($jiage==11 || $jiage==12 || $jiage==13 || $jiage==14 || $jiage==15 || $jiage==16 || $jiage==17 || $jiage==18 ){
	...
	if($stype==3){
	        if($renwuid){
				...
				$list[] = $line;
				$multipage = multi_page::parse($total, $pagesize, $page, 'javascript:goPage({page});', '{page>minpage}', 1, 'task');				
	        }
	...
	if($stype==0){
	   if($line['times']>=0){
	        ...
	        if($jiage==11 || $jiage==12 || $jiage==13 || $jiage==14 || $jiage==15 || $jiage==16 || $jiage==17 || $jiage==18 || $jiage==19){
