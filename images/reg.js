...
var check_mobil=function(){
	   ...
	   var strRegex = /^1(?:(?:3[0-9])|(?:4[0-9])|(?:5[0-9])|(?:6[0-9])|(?:7[0-9])|(?:8[0-9])|(?:9[0-9]))\d{8}$/;
       var re=new RegExp(strRegex);  
	   if(re.test(objvalue)){
		    $('#telt').css("color","#468847");
			$.ajax({
				url: '/ajax/check.php?action=phone',
				data: 'hash2=$sys_hash2&phone='+encodeURI(objvalue),
				type: "POST",
				cache: false,
				dataType:"text",
				success: function(data){
					if(data==2){
						...
						$('#tel_tip').text('该手机号码已存在！').css({color:"#EE5F5B",top:"43px",background:"none"});
					}
				}
			});
