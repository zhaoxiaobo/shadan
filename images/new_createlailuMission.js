    ...
    if( curG < 1 ){
        ...
        $("#btnCilentAdd").click(function(){
           ...
           if($("#cbxIsTaoG").is(":checked")){
	        ...
	        if(!validGoodsUrl||  validGoodsUrl.indexOf("meal")>0){
		        artDialog({content:"对不起，您还未输入商品URL地址或者地址有错误。（特别注意：如果链接错误,请从店铺首页打开产品链接,再复制到平台上！）",id:"alarm",yesText:"确定",noText:"查看URL地址帮助"},function(){ToMao();},function(){window.open("http://www.xozuan.com/bbs/t153/");});
				$(".ui_close").hide();
		        return false;
	        }else{
        ...
        $("#ddlOKDay").change(function(){
            ...
            if( okDay==0 )
                ...
                var addPrice=(minPrice*basePriceDouble + okDay - 0.5) - minPrice;
		...
		function changetype(radioid){
			if(radioid=="rshop"){
				document.getElementById("divkeykey").style.display="none";
				document.getElementById("divkeykeyxian").style.display="none";
				document.getElementById("divkeykeyhou").style.display="none";
				$("#divkey").html("搜索店铺关键字");
				...
				document.getElementById("divkeykey").style.display="none";
				document.getElementById("divkeykeyxian").style.display="block";
				document.getElementById("divkeykeyhou").style.display="block";
				$("#divkey").html("搜索商品关键字");
				$("#divkeytip").html("推荐使用您的宝贝名称，如果您的宝贝名称在淘宝中重名商品过多，我们建议您在宝贝名称后面增加个唯一性字符或编号等信息。");
				$("#divdes").html("搜索提示");
				$("#divdestip").html("请输入提示信息，说明店铺在淘宝搜索结果列表中的位置，商品在店铺首页中的大概位置等等，例如：店铺在搜索结果第二个，商品在店铺首页第二排第一个");
			} else {
				document.getElementById("divkeykey").style.display="block";
				document.getElementById("divkeykeyxian").style.display="none";
				document.getElementById("divkeykeyhou").style.display="none";
				$("#divkey").html("搜索商品关键字");
       ...
       /* $("#cbxIsSetTime1").click(function(){
		...
		if(isVip===false){
			...
			artDialog({content:"目前限制接手人IP功能仅针对川山岭VIP开放！",id:"alarm",yesText:"加入VIP",noText:"返回修改"},
        function(){window.open("/member/vip/");},
...
function ValidPrice(price){
	var nopingtaickjg = $("#nopingtaick").val();
	if (nopingtaickjg==1){
		nopingtaickjgx = true;
	}else
	{
		nopingtaickjgx = false;
	}
    if( price > 4000 )
            artDialog({content:"对不起！只能发布4000元以下的商品。",id:"alarm",yesText:"返回修改",noText:"取消"},
            function(){Init();ToMao();},
            function(){},function(){Init();ToMao();});
    else if( price > curMoney && !tuoguantask && !nopingtaickjgx ){
...
function countPoint() {
   ...
   if (getObj('cbxIsAudit').checked) p += 0.2;
   if (getObj('nopingtaick').checked) p += 0.1;
   if (getObj('cbxIsWW').checked) p += 0.5;
   ...
   if (getObj('isShare').checked) p += 0.1;
   ...
   if (getObj('isLimitCity').checked) p += 0.3;
...
function addconfirm(){
	...
	if (visitWay==3) s +='<p>来路任务方式:信用评价</p>';
	if (visitWay==4) s +='<p>来路任务方式:直通车</p>';
	s += '<p>好评时限：';
	if (okDay == 0) s += '马上好评';
    if (okDay == 1) s += '24小时好评';
    if (okDay == 2) s += '48小时好评';
    ...
    if (getObj('isPlan').checked) s += '<p>定时发布：'+$('#planDate').val()+'分时发布任务</p>';
	if (getObj('isawb').checked) s +='<p>快递单号费用：<em>3</em>元</p>';
	s += '<p>商品价格<em>'+ price_count +'</em>元　　消耗信币<em>'+ point_count +'</em>个</p></div>';
