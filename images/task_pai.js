    ...
    if( result.state == 1 ){
        ...
        for(i=0;i<result.itemList.length;i++){
            ...
            if (result.itemList[i].taoG>0)
                htm+="<em class=\"golda\" title=\"此任务需要"+ result.itemList[i].taoG +"个淘信币\">&nbsp;</em>";
            ...
            if (result.itemList[i].OKDay > 0)
            {
                if (result.itemList[i].mPrice > result.itemList[i].vPrice)
                    htm+="<em style=\"color:#090;\">"+result.itemList[i].mPrice.toFixed(2)+"&nbsp;个信币</em><b>↑&nbsp;发布者增加了任务发布信币"+Number(result.itemList[i].mPrice-result.itemList[i].vPrice).toFixed(2)+"个</b>";
                else
                    htm+="<em style=\"color:#090;\">"+result.itemList[i].mPrice.toFixed(2)+"&nbsp;个信币</em>";
            }
            else
            {
                if (result.itemList[i].mPrice > result.itemList[i].vPrice)
                    htm+="<em>"+result.itemList[i].mPrice.toFixed(2)+"&nbsp;个信币</em><b>↑&nbsp;发布者增加了任务发布信币"+Number(result.itemList[i].mPrice-result.itemList[i].vPrice).toFixed(2)+"个</b>";
                else
                    htm+="<em>"+result.itemList[i].mPrice.toFixed(2)+"&nbsp;个信币</em>";
            }
            htm+="</td>";
            htm+="<td class=\"do\">";
...
function taskin(mid,isDisWW,btn){
	if(!curExam){
        artDialog({content:"您还未通过新手考试，通过考试还能获得1个信币哦！",id:"alarm",fixed:true,yesText:"立即去考试",lock:true},function(){
    ...
    if($("#divIDList input").length==0){
        artDialog({content:"您还未绑定您的互刷买号，或您绑定的互刷买号未启用，请点击此处绑定您的互刷买号。",id:"alarm",fixed:true,yesText:"立即去绑定",lock:true},function(){
            window.open("/task/new/?m=tieBuyer");
        });
        DisabledClose();
        return;
    }
    ...
    $.post("/ajax/MyTaskIn.php",{"taskid":mid}, function(result){
        ...
        if(result.StateCode==0){
            ...
            switch(result.StateCode){
				...
				case 13:{
                    artDialog({content:result.StateMsg,id:"alarm",fixed:true,yesText:"点击进行加V认证",lock:true},function(){
...
function SelectBuyerAccountNew(mid,btn,$accountList){
    ...
    if(dislen==alllen){
        artDialog({content:"享赚温馨提示：您所有的买号都不能接手此任务。",id:"al1",fixed:true,yesText:"关闭，继续抢其它任务",lock:true},function(){
        ...
        artDialog({content:$accountList.html(),title:"为任务"+mid+"选择买号" ,id:"al",fixed:true,lock:true},function(){            
            ...
            if(aId==undefined){
                artDialog({content:"享赚温馨提示：请选择好买号再提交!<br/>如果没有买号，请到【会员中心-绑定买号】里面先绑定买号再接任务",id:"alarm",fixed:true,yesText:"立即去绑定",noText:"重选买号",lock:true},function(){
                    window.open("/task/new/?m=tieBuyer");
                },function(){
                    ...
                    $.post("/ajax/choosebuyer.php",{"taskid":mid,"buyAccount":aId}, function(result){  
                        ...
                        if(result.StateCode==0){
                            artDialog({content:result.StateMsg,id:"alarm",fixed:true,lock:true},function(){
                                window.open("/task/"+result.urlnew+"/?m=taskIn&t=ing","_self");
                            });
                            DisabledClose();
                            return;
                        }else{                       
...
function OpenTrueNameBa(obj){
    ...
    $("#alContent span[id=spanNormalHead]").html(showImage + "淘宝买号（点击查看淘宝买号）").next().show();
    $("#alContent span[id=spanTrueNameHead]").html(hiddenImage + "互刷买号").next().show();
}
function OpenNormalBa(){
    //$("#alContent tr[id=trListTrueName]").hide();
    $("#alContent tr[id=trListNormal]").show();
    $("#alContent span[id=spanNormalHead]").html(hiddenImage + "淘宝买号").next().show();
    $("#alContent span[id=spanTrueNameHead]").html(showImage + "互刷买号（点击查看互刷买号）").next().show();
}
