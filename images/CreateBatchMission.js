        ...
        $("#btnCilentAdd").click(function(){
           ...
           if($("#cbxIsTaoG").is(":checked")){
	        ...
	        if(!IsURL(validGoodsUrl)|| !ValidDomainF(validGoodsUrl)){
		        artDialog({content:"对不起，您还未输入商品URL地址或者地址有错误。（特别注意：此地址为您店铺需要买方拍的商品地址，而非淘宝店地址）",id:"alarm",yesText:"确定",noText:"查看URL地址帮助"},function(){ToMao();},function(){window.open("http://www.chuanshanling.com/bbs/t1288/");});
				$(".ui_close").hide();
		        return false;
	        }else{
...
function addconfirm(){
	...
	s += '<p>商品价格<em>'+ (price_count*txtFCount).toFixed(2) +'</em>元　　消耗信币<em>'+ point_count +'</em>个</p></div>';
	if(curG<point_count){
