    ...
    if( curG < 1){
        ...
        $("#btnCilentAdd").click(function(){
           ...
           if($("#cbxIsTaoG").is(":checked")){
	        ...
	        if(!validGoodsUrl|| !ValidDomainF(validGoodsUrl) || validGoodsUrl.indexOf("meal")>0){
		        artDialog({content:"对不起，您还未输入商品URL地址或者地址有错误。（特别注意：此地址为您店铺需要买方拍的商品地址，而非淘宝店地址）",id:"alarm",yesText:"确定",noText:"查看URL地址帮助"},function(){ToMao();},function(){window.open("http://bao.dazuoyong.com/");});
				$(".ui_close").hide();
		        return false;
	        }else{
       ...
       $("#cbxIsSetTime1").click(function(){
		...
		if(isVip===false){
			...
			artDialog({content:"目前限制接手人IP功能仅针对富淘商盟VIP开放！",id:"alarm",yesText:"加入VIP",noText:"返回修改"},
