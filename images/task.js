...
function SelectBuyerAccountNew(mid,btn,$accountList,IsReal){
    ...
    if(Reallen = AllReallen && IsReal==1){
		...
		artDialog({content:$accountList.html(),title:"为任务"+mid+"选择买号" ,id:"al",fixed:true,lock:true},function(){            
            ...
            if(aId==undefined){
                artDialog({content:"川山岭温馨提示：请选择好买号再提交!<br/>如果没有买号，请到【会员中心-绑定买号】里面先绑定买号再接任务",id:"alarm",fixed:true,yesText:"立即去绑定",noText:"重选买号",lock:true},function(){
                    window.open("/task/tao/?m=tieBuyer");
                },function(){
...
function OpenTrueNameBa(count){
    ...
    $("#alContent span[id=spanNormalHead]").html(showImage + "普通买号（可用"+countputong+"个，点击查看）").next().show();
...
function OpenNormalBa(count){
    ...
    $("#alContent span[id=spanTrueNameHead]").html(showImage + "实名买号（可用"+countshiming+"个，点击查看）").next().show();
}
        ...
        if($("#divIDList input").length==0){
			...
			var waitMList = new Array();  
			 if( result.itemList.length<30 )
				...
				waitMList[10]=result.itemList[10];
                waitMList[11]=result.itemList[11];
                waitMList[12]=result.itemList[12];
                waitMList[13]=result.itemList[13];
				waitMList[14]=result.itemList[14];
                waitMList[15]=result.itemList[15];
				waitMList[16]=result.itemList[16];
                waitMList[17]=result.itemList[17];
				waitMList[18]=result.itemList[18];
				waitMList[19]=result.itemList[19];
				waitMList[10]=result.itemList[10];
                waitMList[21]=result.itemList[21];
                waitMList[22]=result.itemList[22];
                waitMList[23]=result.itemList[23];
				waitMList[24]=result.itemList[24];
                waitMList[25]=result.itemList[25];
				waitMList[26]=result.itemList[26];
                waitMList[27]=result.itemList[27];
				waitMList[28]=result.itemList[28];
				waitMList[29]=result.itemList[29];
            }  
            for(i=0;i<waitMList.length;i++){
                ...
                if( misItem.status == 1 ){
                    ...
                    if( misItem.point < at_txtMinpoint){
					    ShowAMTip(" "+curTimeDes+"，接任务失败！原因是任务奖励的信币低于"+at_txtMinpoint); 
                        continue;
					}	
                    ...
                    if( misItem.times != at_timesBegin && at_timesBegin>10){
                        ShowAMTip(" "+curTimeDes+"，接任务失败！原因是要求确认好评时间不在期望范围");
						continue;
					}	
            ...
            if( MList.length > 0 ){
                ...
                var taskid = mItem.id;
                $.post("/ajax/MyTaskIn.php",{"taskid":taskid,"isauto":1,}, function(result){  
				    if(result.StateCode==0){
                        StopAutoAM();
                        var $accountList=$("#divIDList");
